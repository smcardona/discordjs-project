...
// todo: rebuild this from scratch maybe lol, or maybe just modify it to clean code at least
// todo: cz this right now is very hard to understand
/**
 * ! CODE REWORKED WAY BETTER AND CLEANER
 * * Check src/utils/classes/EnigmaMachine.ts for the updated one
 * * I'm keeping this one because nostalgia only
 */
...
if (alphabet.length % 2 !== 0) throw new Error('Alphabet must have an even length in order for rotors to work: ' + alphabet.length)
...
function runner(txt, settings, processK) {
  ...
  let rotors = machine.rotors.length; // Amount of rotors the user has set
  for (const char of txt) { // Iterates all characters even spaces
    ...
    for (let j = 0; j < rotors * 2 + 1; j++) { // Cycles trough rotors and reflectors
      const cycle = rotors - Math.abs(j - rotors); // 0, 1, 2, ..n, reflector, n.., 2, 1, 0
      ...
      else if (j < rotors) tempL = this.g(tempL, machine.rotors[cycle])
      // Turning cycle when the signal is going backwards trough the rotors
      else if (j > rotors) tempL = this.t(tempL, machine.rotors[cycle])
    }
